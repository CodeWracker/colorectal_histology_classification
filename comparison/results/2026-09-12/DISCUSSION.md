...
Foram construídos 20 mosaicos de validação e 30 de teste, todos 4×4, com dois patches de tumor e quatorze das demais classes. Os 800 patches são únicos dentro de cada split e a posição dos dois positivos é aleatória e registrada. O threshold que maximiza Dice foi escolhido somente na validação de cada método e seed. No teste, PolyGabor obteve AUROC em pixels 0,6408/0,6387 e Dice 0,2594/0,2588; a ResNet obteve AUROC 0,8964/0,8977 e Dice 0,5584/0,5889. A prevalência positiva é 12,5%; o baseline de marcar tudo como tumor tem Dice 0,2222 e um ranking aleatório tem AUROC esperado 0,5.

A diferença média por mosaico, PolyGabor menos ResNet, foi aproximadamente -0,251 de AUROC nas duas seeds. Os intervalos bootstrap de 95% foram -0,302 a -0,203 na seed 42 e -0,307 a -0,199 na 43. Para Dice, as diferenças foram -0,307 e -0,323, também com intervalos inteiramente negativos. Nesse protocolo, portanto, a explicação mais diretamente inspecionável do PolyGabor não resultou na melhor localização: o CAM da ResNet separou melhor os patches tumorais.

![Exemplos dos mapas de localização](localization/localization_examples.png)

Visualmente, o mapa PolyGabor frequentemente cruza patches não tumorais e forma regiões amplas. Nos três exemplos fixados, seu AUROC por mosaico foi 0,549, 0,857 e 0,483, enquanto o CAM ficou em 0,954, 0,895 e 0,900. O CAM também invade vizinhos e seus contornos não coincidem perfeitamente com as bordas dos patches, efeito esperado do campo receptivo e da interpolação. O [relatório específico](localization/README.md) contém AP, IoU, intervalos por mosaico, thresholds e artefatos brutos.

O PolyGabor produziu os 50 mapas em cerca de 53,9–54,9 s por seed, usando aproximadamente 16,5 núcleos de CPU em média durante essa etapa e sem VRAM. As etapas da ResNet levaram 5,7 s na primeira seed, incluindo inicialização da GPU e cargas, e 2,3 s na segunda; a VRAM por processo chegou a aproximadamente 1,22 GB. As métricas consumiram mais tempo que o CAM por ordenar milhões de pixels.

Os mosaicos fornecem máscara exata porque cada bloco herda o rótulo do patch. Isso mede localização de classes em uma colagem artificial, não contornos de tumor numa lâmina. O arquivo local contém os 5.000 patches e não o dataset separado `colorectal_histology_large`; por isso os mapas qualitativos nas dez imagens grandes mencionados na proposta ainda não foram executados.
