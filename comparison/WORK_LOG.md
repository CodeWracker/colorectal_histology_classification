...
Um piloto de uma época no fold `loso_06` (PolyGabor, ResNet ImageNet e YOLO ImageNet) concluiu e passou na auditoria antes da campanha, em diretório separado e depois removido. O relatório também foi exercitado com predições sintéticas nos dez folds.

A campanha terminou em 2026-09-14 com 100 execuções concluídas e nenhuma falha: seed 42 em 3 h 17 min e seed 43 em cerca de 3 h 13 min. A auditoria verificou 278 runs válidas e a validação final passou. As cópias `yolo_dataset` das runs YOLO LOSO foram removidas após cada conclusão, conforme o protocolo.

No macro-F1 das 5.000 predições concatenadas, com média das seeds 42 e 43, o PolyGabor obteve 0,5539, a ResNet-18 aleatória 0,7001, a ResNet-18 ImageNet 0,7994, a YOLO11n aleatória 0,7532 e a YOLO11n ImageNet 0,7893. As quedas em relação ao split aleatório foram 0,239, 0,208, 0,157, 0,109 e 0,162. O split aleatório original superestimava todos os métodos, e o PolyGabor foi o mais afetado. A vantagem relativa do PolyGabor em `source_a` não se reproduziu.

O recall de empty do PolyGabor foi 0,05: quando a origem 06 foi testada, o treino tinha 31 exemplos de empty, todos da origem 10, e quase todos os 590 recortes foram atribuídos a adipose. Sem os recortes empty, seu macro-F1 sobe para 0,6638, ainda abaixo das CNNs (0,7330 a 0,7894). O PolyGabor teve recall maior que todas as CNNs em debris (0,72) e stroma (0,69), e o maior macro-F1 na origem 08 (0,662). No bootstrap por origem, sete das oito comparações seed/CNN tiveram intervalo inteiramente a favor da CNN; a exceção foi a YOLO11n aleatória na seed 42 (−0,005 a 0,256).

## 2026-09-14 — quantização pós-treino das CNNs

A primeira tentativa de adicionar litert-torch, ai-edge-litert e ai-edge-quantizer como extra de `cnn/` rebaixaria typing-extensions, anyio e immutabledict no lock universal da campanha. A mudança foi desfeita, e a quantização ganhou o projeto uv separado `comparison/quantization`, com torch 2.11 CPU, porque o torchao exigido por litert-torch não importa com torch 2.7.1.

Nos pilotos, converter a ResNet Keras 3 por função concreta produziu um arquivo de 67 kB sem pesos, com 13% de concordância e saídas NaN; a exportação por SavedModel resolveu, com 44,7 MB e concordância integral. A YOLO convertida por litert-torch concordou integralmente com as predições salvas. O pré-processamento de inferência passou a usar OpenCV e Pillow, sem TensorFlow ou PyTorch, e a variante float32 manteve concordância de 99,6% a 100% nas condições do teste da YOLO seed 42.

A primeira execução completa foi encerrada pelo sistema por falta de memória depois de quatro dos oito modelos, durante a calibração da ResNet ImageNet seed 43. O script mantinha os nove testes pré-processados e todos os runtimes no mesmo processo. A avaliação passou a pré-processar uma condição por vez, e cada modelo e seed roda em processo próprio; os quatro modelos concluídos foram reaproveitados.

Mesmo assim, mais duas tentativas foram encerradas. O kernel não registrou OOM: quem encerrava era o gerenciador de tarefas em segundo plano da sessão, por pouca memória livre no sistema. A exportação passou a rodar em subprocesso separado da avaliação, e um driver desacoplado da sessão, como o da campanha LOSO, concluiu os oito modelos, o benchmark e o relatório. A variante float32 concordou 100% com as predições originais no teste limpo e pelo menos 99,6% nas perturbações; as predições do benchmark coincidiram integralmente com as da avaliação.

Em LiteRT, a ResNet-18 foi de 44,7 MB em float32 para 22,4 MB com pesos float16 e 11,3 MB em int8; a YOLO11n foi de 6,27 MB para 3,23 MB e 1,73 a 1,81 MB. Com uma exceção, a variação de macro-F1 no teste limpo ficou entre −0,007 e +0,003. A exceção foi a ResNet-18 ImageNet em int8 estático, que caiu de 0,970 para 0,862 na seed 42 e de 0,942 para 0,887 na seed 43, com o recall de complex caindo para 0,32 e 0,44. A ResNet aleatória, com o mesmo procedimento, não perdeu desempenho.

No benchmark da seed 42 com uma thread, a YOLO11n ImageNet em int8 estático levou 0,83 ms por imagem, com pico de RSS de 109 MB, contra 3,3 ms e 825 MB do modelo original em PyTorch e 19 ms e 280 MB do PolyGabor. A ResNet-18 em int8 levou de 4,0 a 4,5 ms, com cerca de 130 MB. Como o pico das variantes LiteRT vem de `ru_maxrss` e o das linhas originais é amostrado, a comparação favorece as linhas originais. Contra essas variantes, o PolyGabor mantém apenas o menor arquivo (0,65 MB contra 1,73 MB da YOLO11n int8), com macro-F1 de 0,79 contra 0,95.
